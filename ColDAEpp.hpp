...
namespace coldae {
    ...
    public:
        result_t COLDAE(systemParameters const &params, options const &opts,
    ...
    private:
        ...
        void SKALE(dmat Z, dmat DMZ, cdvec XI, dmat SCALE, dmat DSCALE) {
            ...
            for (int j = 1; j <= N; ++j) {
                ...
                for (int ICOMP = 1; ICOMP <= NCOMP; ++ICOMP) {
                    double SCAL =
                            (std::abs(Z[IZ - 1 + (j - 1) * MSTAR]) + std::abs(Z[IZ - 1 + (j) * MSTAR])) * .5 + 1.0;
                    int MJ = MT[ICOMP - 1];
                    for (int l = 1; l <= MJ; ++l) {
        ...
        void NEWMSH(int &MODE, dvec XI, cdvec XIOLD,
            ...
            switch (MODE) {
                ...
                case 1: {
                    {
                                    ...
                                    for (int j = 1; j <= NY; ++j) {
                                        for (int J1 = 1; J1 <= NY; ++J1) {
                                            ...
                                            for (int l = 1, ML = 0; l <= NCOMP; ++l) {
                                                ML += MT[l - 1];
                                                FACT += DF[j + NCOMP - 1 + (ML - 1) * NCY] *
                                                        DF[l - 1 + (MSTAR + J1 - 1) * NCY];
                                            }
                        ...
                        for (int j = 1; j <= NTOL; ++j) {
                            ...
                            SLOPE[0] = std::max(SLOPE[0],
                                                pow(std::abs(D2[JJ - 1] - D1[JJ - 1]) * WGTMSH[j - 1] * ONEOVH /
                                                    (1.0 + std::abs(Z[JZ - 1])),
                                                    ROOT[j - 1]));
                        }
                        ...
                        for (int i = 2; i <= NOLD; ++i) {
                            ...
                            for (int j = 1; j <= NTOL; ++j) {
                                ...
                                auto temp = std::abs(D2[JJ - 1] - D1[JJ - 1]) * WGTMSH[j - 1] * ONEOVH /
                                            (1.0 + std::abs(Z[JZ - 1]));
                        ...
                        for (int i = 1; i <= NFXP1; ++i) {
                            ...
                            if (NREGN != 0) {
                                ...
                                for (int j = 1; j <= NREGN; ++j) {
                                    ...
                                    LOLD = LCARRY;
                                    XI[IN - 1] =
                                            XIOLD[LOLD - 1 - 1] + (TEMP - ACCUM[LOLD - 1 - 1]) / SLOPE[LOLD - 1 - 1];
                                }
                            }
                            IN++;
                            ACCL = ACCR;
                ...
                default:
                    throw std::invalid_argument("NEWMSH: Invalid case.\n");
            } // end of switch
        ...
        void LSYSLV(int &MSING, cdvec XI, cdvec XIOLD, dvec Z, dvec DMZ, dvec DELZ, dvec DELDMZ,
            ...
            switch (M1) {
                ...
                default:
                    throw std::invalid_argument("LSYSLV: Invalid case.\n");
            }
        }
        ...
        void GBLOCK(const double H, dmat GI, const int NROW, const int IROW,
            ...
            switch (MODE) {
                ...
                default:
                    throw std::invalid_argument("GBLOCK: Invalid case.\n");
            }
        }
	...
	* **********************************************************************/
        ...
        void APPROX(int &i, double &X, dvec ZVAL, dvec YVAL, dmat A, cdvec coef, cdvec XI,
                    const int n, cdvec Z, cdvec DMZ, const int k, const int ncomp, const int ny, const int mmax,
                    civec M,
            ...
            switch (MODE) {
                ...
                default:
                    throw std::invalid_argument("APPROX: Invalid case.\n");
            }
        }
	...
	* **********************************************************************/
	...
	----------------------------------------------------------------------*/
        ...
        //
        static void
        SHIFTB(cdmat AI, const int NROWI, const int NCOLI, const int LAST, dmat AI1, const int NROWI1,
               const int NCOLI1) {
            //AI(NROWI, NCOLI);
            //AI1(NROWI1, NCOLI1)

            int MMAX = NROWI - LAST; // shadowing
            int JMAX = NCOLI - LAST;
            if (MMAX < 1 || JMAX < 1)
                return;
